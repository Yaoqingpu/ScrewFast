...
- **Vibration and side loads:** makers warn against twisting, pulling, moment loads and impact. Route and support the tube so bending and vibration never reach the joint; use rotary fittings for oscillating lines.
- **Pull-out:** retention depends on tube material, OD, insertion depth and external load — there is no single category rating. If a line will be tugged or bent sharply, a compression fitting is the more robust joint.
- **Reuse:** tube removal is designed to be repeatable — press the collar, withdraw, inspect the end. But a tube scratched or ovalized where the collet gripped should be cut back before reinsertion. Thread-seal reuse limits (some makers allow ~5 reuses on the threaded side) do not apply to the tube interface.
...
**Are push-to-connect fittings airtight?**
Yes, when the tube OD is in tolerance, the surface is undamaged and the tube is fully inserted. Most leaks trace to one of those three.

**Can push-to-connect fittings be used on stainless steel tubing?**
Only if the specific series approves that tube type and size. Do not assume compatibility from the catalog name — check the maker's tube list.

**Why does my polyurethane tube pop out under pressure?**
Check insertion depth first, then OD tolerance. Pressure-swollen polyurethane can also make reinsertion difficult — cut the tube back to a fresh, round section.
